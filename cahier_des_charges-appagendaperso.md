...
- Authentification via Basic Auth (Protocole CalDAV)
- URL de synchronisation : `https://sync.infomaniak.com`
...
│
├── InfamoniakService    → API CalDAV Basic Auth, CRUD événements
...
### 15.1 Connexions
- Identifiants Infomaniak (User + Mot de passe Application pour CalDAV)
...
- [ ] Service Infomaniak (auth Basic + CalDAV événements)
...
- [ ] Résolution des accès CalDAV (si nécessaire)
- [ ] Open source GitHub : décision différée après V1 stable

> **Prochaine étape immédiate** : Valider la connexion CalDAV (URL `sync.infomaniak.com`), puis finaliser la couche d'accès aux données.
